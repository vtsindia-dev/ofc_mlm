                ...
                @foreach($products as $prod)
                    <div class="col-lg-3 col-md-4 col-6  mb-md-5 mb-4">
                        <div class="product-card card bs_1 h-100 border-0 br_10">
                         <a href="#" data-toggle="modal" data-target="#largeModal{{ $prod->id  }}"><img class="card-img-top" src="{{ getImage('assets/images/frontend/product/'. $prod->image, '350x260')}}" alt="Card image cap"></a>
                         
                              
                              <div class="card-body p-md-3 p-2">
                                <h5 class="card-title"><a href="#" data-toggle="modal" data-target="#largeModal{{ $prod->id  }}" class="one_line">{{  $prod->name }}</a></h5>
                                <div class="d-md-flex justify-content-between align-items-center p-1">
                                 ...
                                 @if(Auth::check())
                                    <div class="qty-input">
                                        ...
                                        <input class="product-qty" type="number" id="qty_{{ $prod->id }}" name="product-qty" min="1" max="100" value="1" disabled>
                     ...
                     <div class="modal fade" id="largeModal{{ $prod->id }}" tabindex="-1" role="dialog" aria-labelledby="basicModal" aria-hidden="true">
                        <div class="modal-dialog modal-lg modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="myModalLabel">{{  $prod->name }}</h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                    </button>
                                </div>
                                <div class="modal-body"> 
                                    <img class="card-img-top" src="{{ getImage('assets/images/frontend/product/'. $prod->image, '350x260')}}" alt="Card image cap">
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-theme modal-btn text-white" data-dismiss="modal">Close</button>
                                </div>
                            </div>
                        </div>
                    </div>
